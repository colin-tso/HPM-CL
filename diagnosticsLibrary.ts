...
import { TextmateToken } from "vscode-textmate-languageservice/dist/types/src/services/tokenizer";

export function checkVariableDeclarations(
  ...
  for (const token of tokens) {
    ...
    const isVariable = scopes.some(
      (item) => item.includes("variable.other.cl") || item.includes("variable.declaration.cl")
    );
...
export function checkLabelDeclarations(
  tokens: TextmateToken[],
  diagnostics: vscode.Diagnostic[]
): void {
  enum LabelType {
  ...
  const declaredLabels = {
    [LabelType.GOTO]: new Set<string>(),
    [LabelType.REPEAT]: new Set<string>(),
  };
  const calledLabels = {
    [LabelType.GOTO]: new Set<string>(),
    [LabelType.REPEAT]: new Set<string>(),
  ...
  for (const token of tokens) {
    ...
    switch (true) {
      ...
      case scopes.includes("label.goto.cl"):
        if (
          ...
          if (!declaredLabels[LabelType.GOTO].has(labelName)) {
            // Raise error
            console.error(
              `Label identifier '${labelName}' has not been declared for GOTO statement.`
            );
            pushDiagnostic(
              token,
              `Missing label identifier for GOTO statement '${labelName}'.`,
              diagnostics
            );
          }
        } else {
          console.error(
            `Duplicate label. Label identifier '${labelName}' has already been called for a GOTO statement.`
          );
          pushDiagnostic(
            token,
            `Duplicate label. Label identifier '${labelName}' has already been called for a GOTO statement.`,
      ...
      case declaredLabels[LabelType.GOTO].has(labelName) &&
        ...
        console.error(
          `Duplicate label. Label identifier '${labelName}' has already been declared for a GOTO statement.`
        );
        pushDiagnostic(
          token,
          `Duplicate label. Label identifier '${labelName}' has already been declared for a GOTO statement.`,
      ...
      case scopes.includes("label.repeat.cl"):
        if (
          ...
          if (!declaredLabels[LabelType.REPEAT].has(labelName)) {
            // Raise error
            console.error(
              `Label identifier '${labelName}' has not been declared for REPEAT statement.`
            );
            pushDiagnostic(
              token,
              `Missing label identifier for REPEAT statement '${labelName}'.`,
              diagnostics
            );
          }
        } else {
          console.error(
            `Duplicate label. Label identifier '${labelName}' has already been called for a REPEAT statement.`
          );
          pushDiagnostic(
            token,
            `Duplicate label. Label identifier '${labelName}' has already been called for a REPEAT statement.`,
      ...
      case declaredLabels[LabelType.REPEAT].has(labelName) &&
        (scopes.includes("label.start.cl") || scopes.includes("label.loop.cl")):
        console.error(
          `Duplicate label. Label identifier '${labelName}' has already been declared for a REPEAT statement.`
        );
        pushDiagnostic(
          token,
          `Duplicate label. Label identifier '${labelName}' has already been declared for a REPEAT statement.`,
...
export function checkLabelDeclareOnContinue(
  tokens: TextmateToken[],
  diagnostics: vscode.Diagnostic[]
): void {
  for (const token of tokens) {
...
export function checkContinuationSOL(
  tokens: TextmateToken[],
  diagnostics: vscode.Diagnostic[]
): void {
  for (const token of tokens) {
    ...
    if (
      scopes.includes("invalid.illegal.constant.character.continuation.cl") &&
      !scopes.includes("comment.line.double-dash.cl")
    ) {
      // Raise a lint error
      console.error(
        `Continuation character must be at the start of the line. [checkContinuationSOL]`
      );
      pushDiagnostic(
        token,
        `Continuation character must be at the start of the line.`,
        diagnostics
      );
    }
  }
...
export function checkContinuationSyntax(
  tokens: TextmateToken[],
  diagnostics: vscode.Diagnostic[]
): void {
  var lastErrorLine: number = 0;
  var validLine: number = 0;
  const openBrackets: string[] = ["("];
  const closeBrackets: string[] = [")"];
  const bracketStack: TextmateToken[] = [];
  ...
  for (const token of tokens) {
    ...
    // Raise a lint error
    console.error(
      `Expected continuation character at the start of the line. [checkContinuationSyntax]`
    );
...
export function checkLabelSyntax(tokens: TextmateToken[], diagnostics: vscode.Diagnostic[]): void {
  var lastErrorLine: number = undefined;
  var validLine: number = undefined;
  var labelLine: number = undefined;
  var afterLabel: boolean = false;
  ...
  for (const token of tokens) {
    ...
    if (token.line > labelLine) {
      // Raise a lint error
      console.error(
        `Expected continuation character at the start of the line after label. [checkLabelSyntax]`
      );
      lastErrorLine = token.line;
      afterLabel = false;
      pushDiagnostic(
        token,
        `Expected continuation character at the start of the line after label.`,
        diagnostics
      );
    }
  }
...
export function checkBrackets(tokens: TextmateToken[], diagnostics: vscode.Diagnostic[]): void {
  const stack: TextmateToken[] = [];
  const openBrackets: string[] = ["("];
  const closeBrackets: string[] = [")"];
  const diagnosticMessages: string[] = ["Missing closing bracket.", "Missing opening bracket."];
...
export function checkBLOCKSyntax(tokens: TextmateToken[], diagnostics: vscode.Diagnostic[]): void {
  const blockDeclarations = new Set<TextmateToken>();
  const blockENDs = new Set<TextmateToken>();
  ...
  for (const token of tokens) {
    ...
    if (
      !scopes.includes("entity.name.function.block.begin.cl") &&
    ...
    if (
      ...
      if (!setContainsTokenText(token, blockDeclarations)) {
        ...
        pushDiagnostic(
          token,
          `Duplicate BLOCK or SUBROUTINE identifier '${token.text}'.`,
          diagnostics
        );
      }
    }
    ...
    if (scopes.includes("entity.name.function.end.cl")) {
      if (setContainsTokenText(token, blockENDs)) {
        pushDiagnostic(
          token,
          `Duplicate BLOCK or SUBROUTINE identifier in END statement '${token.text}'.`,
          diagnostics
        );
      } else if (!setContainsTokenText(token, blockDeclarations)) {
        pushDiagnostic(
          token,
          `BLOCK or SUBROUTINE identifier '${token.text}' has not been declared.`,
          diagnostics
        );
  ...
  for (const blockDeclaration of blockDeclarations) {
    if (!setContainsTokenText(blockDeclaration, blockENDs)) {
      pushDiagnostic(
        blockDeclaration,
        `Missing END statement for BLOCK or SUBROUTINE identifier '${blockDeclaration.text}'.`,
        diagnostics
      );
    }
  }
...
export function checkPACKAGESyntax(
  tokens: TextmateToken[],
  diagnostics: vscode.Diagnostic[]
): void {
  var inPackage: boolean = false;
  var afterEND: boolean = false;
  const stack: TextmateToken[] = [];
...
export function checkCUSTOMSyntax(tokens: TextmateToken[], diagnostics: vscode.Diagnostic[]): void {
  var inCUSTOM: boolean = false;
  var afterEND: boolean = false;
  const stack: TextmateToken[] = [];
  ...
  for (const token of tokens) {
    ...
    if (
      !scopes.includes("keyword.control.function.cl") &&
...
function setContainsTokenText(tokenKey: TextmateToken, tokenSet: Set<TextmateToken>): boolean {
...
function setDeleteToken(tokenKey: TextmateToken, tokenSet: Set<TextmateToken>): boolean {
  for (const token of tokenSet) {
    if (token.text === tokenKey.text) {
